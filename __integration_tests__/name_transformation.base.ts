export function nameTransformationTest(
  transform: (secretName: string) => string,
) {
  const dataset = [
  ...
  ].map(([secretName, expectedValue]) => [
    transform(secretName),
    expectedValue,
  ]);

  test.each(dataset)(
    "Secret with name %s test",
    (secretName, expectedValue) => {
