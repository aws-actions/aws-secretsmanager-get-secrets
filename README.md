...
To view the environment variables created from your secrets, turn on debug logging. For more information, see [Enabling debug logging](https://docs.github.com/en/actions/monitoring-and-troubleshooting-workflows/enabling-debug-logging) in the _GitHub Docs_.

To use the environment variables created from your secrets, see [Environment variables](https://docs.github.com/en/actions/learn-github-actions/environment-variables) in the _GitHub Docs_.
...
If two environment variables would end up with the same name, the action fails. In this case, you must specify the names you want to use for the environment variables as _aliases_.
...
You can set the environment variable name by specifying an _alias_, as shown in the following example which creates a variable named `ENV_VAR_NAME`.
...
The following example creates environment variables for all secrets with names that begin with _beta_.
