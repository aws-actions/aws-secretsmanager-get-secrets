...
import { CLEANUP_NAME, LIST_SECRETS_MAX_RESULTS, NameTransformation } from "./constants";
 ...
 */
export async function buildSecretsList(client: SecretsManagerClient, configInputs: string[], nameTransformation: NameTransformation = NameTransformation.Uppercase): Promise<string[]> {
...
export function injectSecret(
    ...
    nameTransformation: NameTransformation = NameTransformation.Uppercase,
 ...
 */
export function transformToValidEnvName(secretName: string, nameTransformation: NameTransformation = NameTransformation.Uppercase): string {
    ...
    switch (nameTransformation) {
        case NameTransformation.Lowercase:
            return secretName.toLowerCase();
        case NameTransformation.Uppercase:
            return secretName.toUpperCase();
        case NameTransformation.None:
            return secretName;
        default:
            throw new Error(`'${nameTransformation}' is unsupported name transformation.`);
    }
}
 ...
 */
export function extractAliasAndSecretIdFromInput(input: string, nameTransformation: NameTransformation = NameTransformation.Uppercase): [undefined | string, string] {
...
/*
 * Converts string to NameTransformation enum.
 */
export function parseNameTransformation(config: string): NameTransformation {
    const nameTransformation = config.toLocaleLowerCase() as NameTransformation;
    if (!Object.values(NameTransformation).includes(nameTransformation)){
        throw new Error(`'${nameTransformation}' is unsupported transformation name. Allowed options are: ${Object.values(NameTransformation).map(x => x.valueOf()).join(', ')}.`);
    }
