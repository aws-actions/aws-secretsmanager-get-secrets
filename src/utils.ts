...
export async function buildSecretsList(
  ...
  for (const configInput of configInputs) {
    if (configInput.includes("*")) {
      const [secretAlias, secretPrefix] = extractAliasAndSecretIdFromInput(
        configInput,
        nameTransformation,
      );
...
export function injectSecret(
  ...
  if (parseJsonSecrets && isJSONString(secretValue)) {
    ...
    for (const k in secretMap) {
      ...
      const newEnvName = [
        tempEnvName ||
          transformToValidEnvName(secretName, nameTransformation, false),
    ...
    const envName = transformToValidEnvName(
      tempEnvName ? tempEnvName : secretName,
      nameTransformation,
    );
 ...
 */
export function transformToValidEnvName(
  secretName: string,
  nameTransformation?: TransformationFunc,
 ...
 */
export function parseTransformationFunction(
  config: string,
): TransformationFunc {
  switch (config.toLowerCase()) {
    ...
    default:
      throw new Error(
        `'${config}' is unsupported transformation name. Allowed options are: 'uppercase', 'lowercase' and 'none'`,
      );
  }
}
