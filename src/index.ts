...
export async function run(): Promise<void> {
  try {
    ...
    const secretConfigInputs: string[] = [
      ...new Set(core.getMultilineInput("secret-ids")),
    ];
    const parseJsonSecrets = core.getBooleanInput("parse-json-secrets");
    const nameTransformation = parseTransformationFunction(
      core.getInput("name-transformation"),
    );

    // Get final list of secrets to request
    core.info("Building secrets list...");
    const secretIds: string[] = await buildSecretsList(
      client,
      secretConfigInputs,
      nameTransformation,
    );
    ...
    core.info(
      "Your secret names may be transformed in order to be valid environment variables (see README). Enable Debug logging in order to view the new environment names.",
    );
    ...
    for (let secretId of secretIds) {
      ...
      [secretAlias, secretId] = extractAliasAndSecretIdFromInput(
        secretId,
        nameTransformation,
      );
      ...
      try {
        const secretValueResponse: SecretValueResponse = await getSecretValue(
          client,
          secretId,
        );
        ...
        if (
          secretAlias === "" &&
          !(parseJsonSecrets && isJSONString(secretValue))
        ) {
    ...
    if (existingCleanupSecrets) {
      secretsToCleanup = [
        ...JSON.parse(existingCleanupSecrets),
        ...secretsToCleanup,
      ];
    }
