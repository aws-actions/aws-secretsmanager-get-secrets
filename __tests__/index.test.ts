...
const SECRET_FOR_BLANK_3 =
  '{"username": "integ", "password": "integpw", "config": {"id2": "example2"}}';
...
jest.mock("@actions/core", () => {
  return {
    ...
    exportVariable: jest.fn(
      (name: string, val: string) => (process.env[name] = val),
    ),
    setSecret: jest.fn(),
  };
});
...
describe("Test main action", () => {
  ...
  test("Retrieves and sets the requested secrets as environment variables, parsing JSON", async () => {
    ...
    const booleanSpy = jest
      .spyOn(core, "getBooleanInput")
      .mockReturnValue(true);
    const multilineInputSpy = jest
  ...
  test("Defaults to correct behavior with empty string alias", async () => {
    ...
    const multilineInputSpy = jest
      .spyOn(core, "getMultilineInput")
      .mockReturnValue([BLANK_ALIAS_INPUT_2, BLANK_ALIAS_INPUT_3]);
    ...
    expect(core.exportVariable).toHaveBeenCalledWith(
      CLEANUP_NAME,
      JSON.stringify(["TEST_BLANK2", "TEST_BLANK3"]),
    );
  ...
  test("Fails the action when an error occurs in Secrets Manager", async () => {
    ...
    const multilineInputSpy = jest
      .spyOn(core, "getMultilineInput")
      .mockReturnValue([TEST_NAME, TEST_INPUT_3, TEST_ARN_INPUT]);
  ...
  test("Fails the action when multiple secrets exported the same variable name", async () => {
    ...
    const multilineInputSpy = jest
      .spyOn(core, "getMultilineInput")
      .mockReturnValue([TEST_NAME, TEST_INPUT_3, TEST_ARN_INPUT]);
    const nameTransformationSpy = jest
      .spyOn(core, "getInput")
      .mockReturnValue("uppercase");
  ...
  test("Keep existing cleanup list", async () => {
    ...
    process.env = {
      ...process.env,
      SECRETS_LIST_CLEAN_UP: JSON.stringify([
        "EXISTING_TEST_SECRET",
        "EXISTING_TEST_SECRET_DB_HOST",
      ]),
    };
  ...
  test("handles invalid timeout string", async () => {
    const timeoutSpy = jest
      .spyOn(core, "getInput")
      .mockReturnValue(INVALID_TIMEOUT_STRING);

    smMockClient.on(GetSecretValueCommand).resolves({ SecretString: "test" });
  ...
  test("handles invalid timeout value", async () => {
    const timeoutSpy = jest
      .spyOn(core, "getInput")
      .mockReturnValue(INVALID_TIMEOUT);
