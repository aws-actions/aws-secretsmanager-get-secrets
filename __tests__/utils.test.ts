...
const NESTED_JSON_SECRET =
  '{"host":"127.0.0.1", "port": "3600", "config":{"db_user":"testuser","db_password":"testpw","options":{"a":"YES","b":"NO", "c": 100 }}}';

const VALID_ARN_1 =
  "arn:aws:secretsmanager:us-east-1:123456789000:secret:test1-aBcdef";
const TEST_NAME_1 = "test/secret1";

const VALID_ARN_2 =
  "arn:aws:secretsmanager:ap-south-1:123456789000:secret:test2-aBcdef";
...
const INVALID_ARN =
  "aws:secretsmanager:us-east-1:123456789000:secret:test3-aBcdef";
...
describe("Test secret value retrieval", () => {
  ...
  test("Throws error on invalid list secrets response ", async () => {
    ...
    await expect(
      getSecretsWithPrefix(smClient, "test", false),
    ).rejects.toThrow();
  });
...
describe("Test secret parsing and handling", () => {
  ...
  test("Stores a variable for nested JSON key values when parseJson is true", () => {
    ...
    expect(core.exportVariable).toHaveBeenCalledWith(
      "TEST_SECRET_CONFIG_DB_USER",
      "testuser",
    );
    expect(core.exportVariable).toHaveBeenCalledWith(
      "TEST_SECRET_CONFIG_DB_PASSWORD",
      "testpw",
    );
    expect(core.exportVariable).toHaveBeenCalledWith(
      "TEST_SECRET_CONFIG_OPTIONS_A",
      "YES",
    );
    expect(core.exportVariable).toHaveBeenCalledWith(
      "TEST_SECRET_CONFIG_OPTIONS_B",
      "NO",
    );
    expect(core.exportVariable).toHaveBeenCalledWith(
      "TEST_SECRET_CONFIG_OPTIONS_C",
      "100",
    );
  });
  ...
  test("Separates an alias from an id if provided", () => {
    ...
    expect(
      extractAliasAndSecretIdFromInput("SECRET_ALIAS, test/secret"),
    ).toEqual(["SECRET_ALIAS", "test/secret"]);
    expect(
      extractAliasAndSecretIdFromInput(`ARN_ALIAS,${VALID_ARN_1}`),
    ).toEqual(["ARN_ALIAS", VALID_ARN_1]);
  });

  test("Returns undefined for alias if none is provided", () => {
    expect(extractAliasAndSecretIdFromInput("test/secret")).toEqual([
      undefined,
      "test/secret",
    ]);
    expect(extractAliasAndSecretIdFromInput(VALID_ARN_1)).toEqual([
      undefined,
      VALID_ARN_1,
    ]);
  });
  ...
  test("Returns empty string for alias if none is provided, but comma delimited", () => {
    ...
    expect(extractAliasAndSecretIdFromInput(" , " + VALID_ARN_3)).toEqual([
      "",
      VALID_ARN_3,
    ]);
  });
  ...
  test("Transformation function is applied", () => {
    expect(transformToValidEnvName("secret3", (x) => x.toUpperCase())).toBe(
      "SECRET3",
    );
  });
  ...
  test('Test valid nested JSON { "a": "yes", "options": { "opt_a": "yes", "opt_b": "no"} } ', () => {
    expect(
      isJSONString(
        '{ "a": "yes", "options": { "opt_a": "yes", "opt_b": "no"} }',
      ),
    ).toBe(true);
  });
  ...
  test.each([
    ...
    "NameTransformation parsing of string %s should pass.",
    (name: string, transformation: TransformationFunc) => {
      const sampleString = "$abcdEFGijk_";
      const parsedTransformation = parseTransformationFunction(name);
      expect(parsedTransformation(sampleString)).toEqual(
        transformation(sampleString),
      );
    },
  );

  test.each(["something", ""])(
    "NameTransformation parsing of string %s should fail.",
    (input) => {
